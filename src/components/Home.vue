<template>
  <div
    class="min-h-[80vh] flex flex-col justify-center pl-4 sm:pl-6 md:pl-12 lg:pl-24 pr-4 sm:pr-6 relative"
  >
    <div class="max-w-3xl">
      <div
        :class="[
      ...
      >
        I'm
        <span :class="['font-bold', themeClasses.themeClass('text-black', 'text-white')]">{{
          info.name
        }}</span>. A <span :class="themeClasses.classes.textMuted()">{{ info.role }}</span> with
        <span :class="['font-bold', themeClasses.themeClass('text-blue-700', 'text-white')]">{{ info.totalExperience.years }}+ years</span>
        of experience who loves turning messy problems into clean,
        <span
          :class="[
            'px-1 mx-1',
            themeClasses.themeClass('bg-yellow-200 text-gray-900', 'bg-yellow-900 text-yellow-100'),
          ]"
        >optimized code.</span>
        Specializing in
